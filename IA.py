...
program_name = Label(main, text="+------- Welcome to our game ------+", bg="#56aed6",fg= "red", font=("Roboto", 25))
program_name.pack(side=TOP)

ask = Label(main, text = "PRESS START BUTTON, " "\n AND COMBINE PLEASANT WITH USEFUL " ,  bg="#56aed6", fg="black", font=("Nyala ", 25))
ask.pack(anchor=CENTER, pady = 100)

def start():
    main.destroy()
    main_filee()

b= Button(main, text="START", bg="PURPLE", font=("Roboto", 30),command= start)
...
def main_filee():
    ...
    a = Label(main, text="Choose game ", bg="#56aed6", font=("Roboto", 20))
    a.pack(side=TOP, pady=34)
...
def add_word():
    ...
    def save_word():
        ...
        messagebox.showinfo("Word saved!", f"{word} : {definition}")
    ...
    save_button = tk.Button(root, text="Save", command=save_word)
    save_button.grid(row=2, column=1, pady=10)

    root.mainloop()
add_button= Button(main,text="Add new word",bg='blue',font=("Roboto", 15),command= add_word )
